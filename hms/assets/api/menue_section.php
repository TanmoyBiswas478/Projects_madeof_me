...
{
 unset($data1['op']);
 $obj->insert_db('navigation', $data1);
}

// View data into the the table 
else if($op=='view')
{
 $obj->view_db('navigation'); 
}
...
else if($op=='edit')
{
 ...
 $where = "id =".$data1['id'];
 unset($data1['idd']);
 $obj->update_db('navigation', $data1,$where);
}
...
{
 unset($data1['op']);
 $obj->delete_db('navigation', $data1);
}
...
else if($op=='select_condition')
{
 ...
 $where = "id =".$data1['idd'];
 unset($data1['idd']);
 $obj->select_db('navigation', $data1,$where);
}
?>