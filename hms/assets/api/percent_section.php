...
else if($op=='edit')
{
 ...
 $where = "id =".$data1['id'];
 unset($data1['id']);
 $obj->update_db('percent', $data1,$where);
}
...
else if($op=='select_condition')
{
 ...
 $where = "id =".$data1['idd'];
 unset($data1['idd']);
 $obj->select_db('percent', $data1,$where);
}

else if($op=='aview')
{
 unset($data1['op']);
 //$sql="select l.user,s.name from login l,percent s where l.login_status='1' and l.user=s.ph";
