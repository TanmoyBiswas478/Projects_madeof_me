...
{
 unset($data1['op']);
 $obj->insert_db('bed_group', $data1);
}
else if($op=='view')
{
 unset($data1['op']);
 $sql="select * from bed_group";
...
else if($op=='edit')
{
 ...
 $where = "id =".$data1['id'];
 unset($data1['id']);
 $obj->update_db('bed_group', $data1,$where);
}
...
else if($op=='select_condition')
{
 ...
 $where = "id =".$data1['idd'];
 unset($data1['idd']);
 $obj->select_db('bed_group', $data1,$where);
}

else if($op=='aview')
{
 unset($data1['op']);
 //$sql="select l.user,s.name from login l,bed_group s where l.login_status='1' and l.user=s.ph";
