...
{
 unset($data1['op']);
 $obj->insert_db('ref_doctor', $data1);
}
else if($op=='view')
{
 unset($data1['op']);
 $sql="select * from ref_doctor";
...
else if($op=='edit')
{
 ...
 $where = "id =".$data1['id'];
 unset($data1['id']);
 $obj->update_db('ref_doctor', $data1,$where);
}
...
{
 unset($data1['op']);
 $obj->delete_db('ref_doctor', $data1);
}
...
else if($op=='select_condition')
{
 ...
 $where = "id =".$data1['idd'];
 unset($data1['idd']);
 $obj->select_db('ref_doctor', $data1,$where);
}

else if($op=='aview')
{
 unset($data1['op']);
 //$sql="select l.user,s.name from login l,ref_doctor s where l.login_status='1' and l.user=s.ph";
