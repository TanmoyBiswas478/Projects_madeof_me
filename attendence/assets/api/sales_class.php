...
class sales {
 ...
 public function addsales($inv,$data1,$data2)
 {
  ...
  try 
  {
   ...
   $check_query = "SELECT COUNT(*) as count FROM sales_master WHERE invno = '$inv'";
...
public function view_db($table,$inv){
  $sql="select * from $table where invno= '$inv'";
...
public function gtotal1($invs){
  $sql="SELECT sum(tax) tax,sum(`gstamt`) gstamt,sum(`total`) total FROM sales WHERE invno= '$invs'";
  $res=$this->mysqli->query($sql); 
  $output=array();
  while($row=$res->fetch_assoc())
  {
  ...
  $cgsta=$gstamt/2;
  $igsta=0.00;
  $sql="update sales_master set tax='".$tax."',cgsta='".$cgsta."',sgsta='".$cgsta."',igsta='".$igsta."',total='".$total."' WHERE invno= '$invs'";
  $res=$this->mysqli->query($sql); 
  echo json_encode($output);
}
...
// function for getting the existing data from client table 
public function client($ph){
  ...
  if ($check_row['count'] > 0) 
  {
   $sql="select * from client where phone= '$ph'";
   $res=$this->mysqli->query($sql); 
   while($row=$res->fetch_assoc())
   {
    $output[]=$row;
    ...
    $output=array(array("name"=>"","cid"=>$cid,"address"=>"","gender"=>""));
  }
  echo json_encode($output); 
}
...
public function service_search($service,$type){
  $output=array();
  $sql="select price,gst from service where iname='".$service."' and type='".$type."'";
  $res=$this->mysqli->query($sql); 
